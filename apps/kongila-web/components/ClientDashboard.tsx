    ...
    <div className="dashboard-shell" style={{ display: 'flex', minHeight: '100vh', background: '#F8FAFC', fontFamily: 'var(--font-display, Inter, sans-serif)' }}>
      ...
      {showIntakeModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.3)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '24px' }}>
          <div style={{ position: 'relative', background: '#FFFFFF', borderRadius: '24px', padding: '16px', width: '100%', maxWidth: '850px', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)', maxHeight: '95vh', overflowY: 'auto' }}>
            {/* Close X Button */}
            <button 
              onClick={() => setShowIntakeModal(false)}
              style={{ position: 'absolute', top: '24px', right: '24px', background: 'var(--bg-secondary)', border: 'none', borderRadius: '50%', width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'var(--text-secondary)', zIndex: 10 }}
            >
              ✕
            </button>
            <SmartIntakeForm
